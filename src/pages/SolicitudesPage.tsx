...
export function SolicitudesPage({ gestion = false }: { gestion?: boolean }) {
  ...
  async function enviarRespuesta(event: FormEvent) {
    ...
    try { await responderSolicitud(seleccionada.id, respuesta, adjuntosRespuesta); setRespuesta(""); setAdjuntosRespuesta([]); await abrir(seleccionada.id); await cargar(); }
  ...
  async function guardarGestion() {
    ...
    try { await actualizarGestionSolicitud(seleccionada.id, gestionForm); setSuccess("Gestión actualizada."); await abrir(seleccionada.id); await cargar(); }
  ...
  return <section className="page solicitudes-page">
    <header className="page-header"><div><span className="eyebrow">{gestion ? "Administración" : "Mi portal"}</span><h1>{gestion ? "Gestión de solicitudes" : "Mis solicitudes"}</h1><p>{gestion ? "Revisa, prioriza, responde y cierra los casos reportados por el equipo." : "Reporta errores, solicitudes o mejoras y consulta aquí su avance."}</p></div>
