...
export function SolicitudesPage({ gestion = false }: { gestion?: boolean }) {
  ...
  return <main className={gestion ? "page solicitudes-page" : "professional-portal-page solicitudes-portal-page"}>
    ...
    <div className="solicitudes-list">{items.map((item) => { const cerradoParaProfesional = !gestion && item.estado === "cerrado"; return <button type="button" className={`solicitud-row ${cerradoParaProfesional ? "cerrada" : ""}`} key={item.id} disabled={cerradoParaProfesional} onClick={() => abrir(item.id)}><span className={`solicitud-status ${item.estado}`}>{estados[item.estado]}</span><div><strong>#{item.id} · {item.asunto}</strong><span>{etiquetas[item.categoria]} · {gestion ? item.profesional_nombre : "Actualizado"} {fecha(item.actualizado_en)}</span>{cerradoParaProfesional && <small className="solicitud-closed-note">Cerrada · no disponible para nuevas acciones</small>}</div><small className={`priority ${item.prioridad}`}>{item.prioridad}</small></button>; })}{items.length === 0 && <div className="empty-state">No hay solicitudes {gestion && filtro ? "con este estado" : "registradas"}.</div>}</div>
