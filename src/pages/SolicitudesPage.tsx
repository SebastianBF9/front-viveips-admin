...
import { actualizarGestionSolicitud, crearSolicitud, downloadBlob, listarGestoresSolicitudes, listarMisSolicitudes, listarSolicitudesGestion, obtenerSolicitud, responderSolicitud } from "../api";
...
export function SolicitudesPage({ gestion = false }: { gestion?: boolean }) {
  ...
  async function cargar() {
    ...
    try {
      if (gestion) {
        const [solicitudes, responsables] = await Promise.all([listarSolicitudesGestion(filtro), listarGestoresSolicitudes()]);
        setItems(solicitudes.solicitudes || []);
        setGestores(responsables.gestores || []);
      } else {
        setItems((await listarMisSolicitudes()).solicitudes || []);
      }
    }
  ...
  return <main className={gestion ? "page solicitudes-page" : "professional-portal-page solicitudes-portal-page"}>
    ...
    <div className="solicitudes-list">{items.map((item) => <button type="button" className="solicitud-row" key={item.id} onClick={() => abrir(item.id)}><span className={`solicitud-status ${item.estado}`}>{estados[item.estado]}</span><div><strong>#{item.id} · {item.asunto}</strong><span>{etiquetas[item.categoria]} · {gestion ? item.profesional_nombre : "Actualizado"} {fecha(item.actualizado_en)}</span>{!gestion && item.estado === "cerrado" && <small className="solicitud-closed-note">Cerrada · solo lectura</small>}</div><small className={`priority ${item.prioridad}`}>{item.prioridad}</small></button>)}{items.length === 0 && <div className="empty-state">No hay solicitudes {gestion && filtro ? "con este estado" : "registradas"}.</div>}</div>
